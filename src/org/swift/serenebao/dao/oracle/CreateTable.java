...
//						}
					...
					{
						Table tableLabel = (Table) clazz.getAnnotation(Table.class);
						create_table_sql += isEmpty(tableLabel.tablespace())?"":" TABLESPACE "+tableLabel.tablespace();
...
//						auto_crement = tableLabel.auto_increment();
					...
					String test = "SELECT count(*) FROM tab WHERE tname='"+table_name.toUpperCase()+"'";
					log.info(test);
					ResultSet rs = connect.executeQuery(test.toUpperCase());
					rs.next();
					int row = rs.getInt(1);
					if(row==0)
					{
