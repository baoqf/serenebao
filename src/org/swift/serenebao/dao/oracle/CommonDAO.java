 ...
 */
package org.swift.serenebao.dao.oracle;
...
public class CommonDAO extends BaseCommonDAO{
	...
	protected String getLimitString(String sql, int row_min, int page_size) {
		...
		int row_max = row_min+page_size;
		int nOrder = sql.indexOf(" order by ");
		int nWhere = sql.indexOf(" where ");
		if(nOrder>1)
		{
			pagingSelect.append("select * from (SELECT A.*, rownum r FROM (");
			pagingSelect.append(sql);
			pagingSelect.append(" ) A WHERE rownum <= "+row_max+") B where r>"+ row_min);
		}
		else
		{
			if(nWhere>1)
			{
				pagingSelect.append(sql);
				pagingSelect.append(" and rownum>"+row_min+" and rownum<="+row_max);
			}
			else
			{
				pagingSelect.append(sql);
				pagingSelect.append(" where rownum>"+row_min+" and rownum<="+row_max);
			}
		}
	...
	public String insert(Object object, DBConnect connect) throws Exception {
		...
		super.putObj2Cache(iden,object);
		iden = super.getIdValue(object);
	...
	public Long getSeqCurValue(String seq) throws Exception {
		...
		try {
			...
			id = (Long)this.getOneRowColBySql(new Query(sql),Long.class,  connect2);
		} finally {
			connect2.close();
		}
		return id;
